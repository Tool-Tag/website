...
const labels=['Material / espesor','Equipo / módulo','Operación','Potencia','Velocidad','Letras / frame','Intervalo','Pasadas','Escaneo','Aire / enfoque','Resultado / notas','Estado','Fecha','Acciones'];
function cell(tr,text){const td=document.createElement('td');td.setAttribute('role','cell');td.setAttribute('data-label',labels[tr.children.length]);td.textContent=text;if(text==='—'||text==='No registrado')td.className='unrecorded';tr.append(td);return td;}
function render(){
 ...
 for(const row of shown){const tr=document.createElement('tr');tr.setAttribute('role','row');cell(tr,row.material+(row.thickness!==''?' · '+row.thickness+' mm':''));cell(tr,row.machine||'—');cell(tr,row.operation);cell(tr,row.power+' %');cell(tr,row.speed+' '+row.unit+(row.unit==='mm/min'?' ('+Number((row.speed/60).toFixed(3))+' mm/s)':''));cell(tr,[(row.letterHeight?'Letras: '+row.letterHeight+' mm':''),(row.frameWidth||row.frameHeight?'Frame: '+(row.frameWidth||'—')+' × '+(row.frameHeight||'—')+' mm':'' )].filter(Boolean).join(' · ')+(row.dimensionsApprox&&(row.letterHeight||row.frameWidth||row.frameHeight)?' (± aprox.)':'')||'—');cell(tr,row.interval?row.interval+' mm':'—');cell(tr,row.passes);cell(tr,row.scan==='No registrado'?'—':row.scan);cell(tr,row.air+(row.focus?' · '+row.focus:''));cell(tr,row.notes||'—');cell(tr,row.status).className='state';cell(tr,row.date||'—');
